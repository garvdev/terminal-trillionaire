                ...
                case trade_type
                    when :buy
                        ...
                        while true
                            ...
                            (print tty_cusor.clear_line; next) if trade_qty == 0
                        ...
                        puts; confirmed = tty_prompt.yes?("Confirming that you would like to purchase #{number_comma(trade_qty).light_cyan} shares of #{trade_ticker.to_s.light_blue}, at a unit price of #{("$"+number_comma(locked_price)).light_green} for a total of #{("$"+number_comma(locked_price*trade_qty)).light_green}?") {|q| q.suffix "y/n"}
                    ...
                    when :sell
                        ...
                        puts "How much #{trade_ticker.to_s.light_blue} would you like to sell today?"
                        
                        i = 0
                        while true
                            trade_qty = Input.get("int")
                            (print tty_cusor.clear_line; next) if trade_qty == 0
                            break if trade_qty <= holdings[trade_ticker]
                            i == 0 ? (puts "You don't have that much #{trade_ticker.to_s.light_blue} to sell! Please try again with a different number.") :(print tty_cursor.clear_lines(2))
                            i += 1
                        end

                        puts; confirmed = tty_prompt.yes?("Confirming that you would like to sell #{number_comma(trade_qty).light_cyan} shares of #{trade_ticker.to_s.light_blue}, at a unit price of #{("$"+number_comma(locked_price)).light_green} for a total of #{("$"+number_comma(locked_price*trade_qty)).light_green}?") {|q| q.suffix "y/n"}
                        puts; confirmed ? (tty_cursor.invisible{20.times {sleep 0.1;tty_bar.advance}}; puts "Trade executed successfully!"; yield [trade_ticker, -trade_qty, locked_price] ; yield [:CASH, (trade_qty*locked_price), 1]) : next
                        STDIN.iflush
                        puts; tty_prompt.yes?("Would you like to execute further trades?") {|q| q.suffix "y/n"} ? next : break 
                end
