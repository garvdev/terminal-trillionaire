        ...
        module TradeLog
            def self.show(user)
                ...
                tty_cursor.invisible do
                    ...
                    trades.map! {|x| [x[0],x[1],"#{number_comma(x[3].to_f)} ","#{number_comma(x[2].to_f)} ","#{number_comma((x[2]*x[3]).to_f)} "]}
                    ...
                    tty_table = TTY::Table.new ["  ID  ",
                                                ...
                                                " Total Value "],
                                                trades
                    separated_table = tty_table.render(:unicode, alignments: [:center, :center, :right, :right, :right]) do |renderer|
                        renderer.border.separator = ->(row) {row == 0 || (row+1) % 2 == 0}
                        renderer.filter = ->(val, row_index, col_index) do
                            return val.bold if row_index == 0
                            row_index > 0 && col_index == 4 ? val.to_i >= 0 ? val.to_s.light_green : val.to_s.light_red : val
                        end
                    end
