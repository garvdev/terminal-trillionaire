        ...
        t = Thread.new do
          while true
            ...
            yield.each_pair do |k, v|
              current_prices[k] = (v[:current]).to_s
            end
            ...
            win.addstr("\n#{table_main.render(:unicode,
                                              alignments: %i[center right right right right right right
                                                             right])}")
            win.addstr("\n\n\nPortfolio Breakdown")
            win.addstr("\n#{pie_chart.render}")
            win.addstr('(Note - You may wish to adjust your terminal font if the chart is distorted.)')
            win.addstr("\n\n\nPortfolio Summary")
            win.addstr("\n#{table_summary.render(:unicode, alignments: %i[right right])}")
            win.refresh
          end
        end
